  ...
  const json = [
    {
      ...
      target: {
        ...
        sha256_cert_fingerprints: [ANDROID_SHA_HEX_VALUE],
      },
    },
  ];
...
app.get("/passkeys", authenticateToken, async (req: Request, res: Response) => {
  // @ts-ignore
  const user = req.user;
  const credentials = await PasskeyService.getUserCredentials(
    user.id as string
  );
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const { user, verified, token } = await PasskeyService.verifyAuthentication(
      credentials,
      challenge
    );
...
app.post(
  ...
  async (req: Request, res: Response) => {
    ...
    const user = req.user;
    const { credentials, challenge } = req.body;
    const { verified, passkey } = await PasskeyService.verifyRegistration(
      user.id,
      credentials,
      challenge
    );
...
app.post("/email-sign-in", async (req: Request, res: Response) => {
  console.log("Request to /email-sign-in with email: ", req.body.email);
...
app.post("/complete-sign-in", async (req: Request, res: Response) => {
  const { email, code } = req.body;
  const { user, token } = await signIn.completeSimplerEmailSignIn({
    email,
    code,
  });
