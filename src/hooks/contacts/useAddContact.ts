...
const useAddContact = () => {
    ...
    const { mutate: addNewContact, isPending, isSuccess } = useMutation({
        mutationFn: addContact,
        onSuccess(contact) {
            queryClient.setQueryData(['contacts'], (contacts: Contact[]) => [...(contacts ?? []), contact].sort((a, b) => a.name.localeCompare(b.name)))
        },
    })
