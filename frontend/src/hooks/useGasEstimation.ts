...
export function formatUoEstimation(
  ...
  const maxFeePerGas: bigint =
    typeof uo.maxFeePerGas === "string"
      ? hexToBigInt(uo.maxFeePerGas)
      : uo.maxFeePerGas;
  const estimation =
    (preVerifGas + callGasLimit + verificationGas) * maxFeePerGas;
...
export function useGasEstimationWithoutPaymaster({
  address,
  amount,
  tokenAddress,
}: {
...
export function useERC20GasEstimation({
  address,
  amount,
  tokenAddress,
}: {
  ...
  const estimateGas = useCallback(async () => {
    ...
    const { formattedInUsdc } = formatUoEstimation(
      uo,
      tokenQuotes[0]?.postOpGas,
      tokenQuotes[0]?.exchangeRate
    );
    const formattedUsdcBalance = ethers.formatUnits(usdcTokenBalance, 6);
    const intPartOfGas = parseInt(formattedInUsdc!.split(".")[0]) + 1.5;
    const intPartOfBalance = Math.floor(
      parseInt(formattedUsdcBalance.split(".")[0])
    );

    const enoughInUsdc = intPartOfGas <= intPartOfBalance;
    return {
      formattedInUsdc: intPartOfGas.toString(),
