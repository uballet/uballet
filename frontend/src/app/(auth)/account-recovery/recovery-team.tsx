...
export default function RecoverWithTeamScreen() {
    ...
    const enoughBalance = estimationQuery.data!.balance >= estimationQuery.data!.totalEstimation
    return (
        <SafeAreaView className="flex-1 items-center">
            ...
            <View className="w-3/4 p-4 border rounded-md mt-4">
                ...
                <Text className="text-xl font-bold">{estimationQuery.data?.totalEstimation.slice(0, 8)} ETH</Text>
            </View>
            <View className="w-3/4 p-4 border rounded-md mt-4 flex-row justify-between">
                <View>
                    ...
                    <Text className="text-xl font-bold">{estimationQuery.data?.balance.slice(0, 8)} ETH</Text>
                </View>
                <View className="justify-center items-center pr-8">
                    {enoughBalance && <MaterialIcons name="check-circle-outline" size={36} color="green" />}
                    {!enoughBalance && <MaterialIcons name="cancel" size={36} color="red" />}
                </View>
            </View>
            {!enoughBalance && <Text className="text-red-500 mt-2">Not enough balance to request recovery</Text>}
            ...
            {myRecoveryTeam && !pendingRequest && (
                <Button mode="contained" disabled={requestRecoveryMutation.isPending || !enoughBalance} loading={requestRecoveryMutation.isPending} className="w-3/4 mt-8" onPress={() => requestRecoveryMutation.mutate()}>
                    Request Recovery
                </Button>
            )}
